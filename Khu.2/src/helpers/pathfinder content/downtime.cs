namespace OpenRoadHelper
{
    public static partial class PF2E
    {
        public abstract class Downtime
        {
            ...
            public short[,] SetupTime { get; } =
                new short[,] { { 4, 6 }, { 3, 5 }, { 3, 5 }, { 2, 4 } };
            public double[,] IncomeTable { get; } =
                new double[,]
                {
        ...
        public class Crafting : Downtime
        {
            ...
            public Crafting(
                ...
                float cost
            ) : base(modifier, charLevel, proficiency)
            {
                _maxDays = maxDays;
                _cost = cost;
        ...
        public class EarnIncome : Downtime
        {
            ...
            public EarnIncome(int daysSpent, short modifier, Proficiency proficiency, short level)
                : base(modifier, level, proficiency)
            {
                _daysSpent = daysSpent;
                _difficulty = (short)PF2E.IncomeTable[_level, 0];
                roll = new();
                Perform();
            }
            ...
            public override void Perform()
            {
                ...
                int level = _level;
                if (result.SuccessIndex == (int)PF2E.SuccessIndex.CritSuccess)
                {
                    level++;
                }
                level = Math.Clamp(level, 0, 21);

                _earned = (float)IncomeTable[level, (int)_proficiency] * 3.0f;
                _earned *= _daysSpent;
                if (result.SuccessIndex == (int)PF2E.SuccessIndex.CritFail)
                {
                    _earned = 0;
                    _daysSpent = 0;
                }
