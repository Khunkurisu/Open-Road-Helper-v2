...
namespace Characters {
	public class Character {
		...
		private readonly Heritage _heritage;
		...
		public Character (IUser user, string name, string desc, string rep, int age, float height, float weight, int birthday,
			Months birthmonth, int birthyear, Ancestry ancestry, Heritage heritage, Class charClass, int level,
			Generation generation, float currency, int downtime, string color, List<string> avatars, string sheet, ulong messageId) {
			Total++;
			_id = Total;
			_user = user;
			_name = name;
		...
		public string GetBirthDate () {
			return $"{GenericHelpers.Ordinal(_birthDay * 1.0f)} {_birthMonth} {_birthYear} AR";
		}
